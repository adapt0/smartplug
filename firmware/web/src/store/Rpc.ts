...
export default function(store: any) {
  let lastGitRevision: string | undefined;
  ...
  store.registerModule('Rpc', {
    ...
    mutations: {
      connectionState(state: any, data: any) {
        if (data === false) {
      ...
      connectionUpdate(state: any, data: any) {
        // console.log('connectionUpdate', state, data);
        mergeDeep(state.data, data);
      },
      wattage(state: any, power: number) {
        state.wattage = [
    ...
    actions: {
      relay(context: any, state: any) {
        return rpcSocket.request('relay', state);
      },
      network(context: any, state: any) {
